...
def get_item_details(item_code):
    ...
    try:
        ...
        frappe.throw(frappe._("Failed to fetch Bin data from the server."), title=frappe._("Database Error"))
    ...
    try:
        ...
        if not last_invoice_data:
            ...
            supplier= frappe.db.get_value("Purchase Invoice" , last_invoice_data[0].get("parent") , "supplier_name" )
       
    except Exception as e:
        print(e)
        frappe.log_error(message=str(e), title="Get Last Purchase Details Error")
        frappe.throw(frappe._("Failed to retrieve last purchase details from the server."), title=frappe._("Database Error"))
    ...
    try:
        ...
        if not last_invoice_data:
            ...
            last_selling_rate = last_invoice_data[0].get("rate") if last_invoice_data else 0.0
...
def quotation_update(doc , method):
    ...
    if (total_item_amount != 0 and total_net_item_amount != 0):
        for i in doc.items:
            i.rate = i.custom_sq_amount + (i.custom_sq_amount/total_item_amount * total_expenses) / i.qty
            i.net_rate = i.custom_sq_net_amount + (i.custom_sq_net_amount/total_net_item_amount * total_expenses) / i.qty
...
def create_je_from_service_expence(doc , method):
    if doc.custom_service_expense:
        ...
        default_credit_account = frappe.db.get_value("Company", company, "custom_default_service_expense")
        
        if not default_credit_account:
            frappe.throw(f"Please set the default service expense account in Company: {company}")
        ...
        je.company = company
        je.posting_date = doc.transaction_date # Assuming 'posting_date' exists in your parent DocType
...
def check_quotation_linked(doc ):
    ...
    for a in sq.items:
        ...
        if frappe.db.get_value("Material Request" , mr , "custom_dc_refrance") :
            qoutation_name = frappe.db.get_value("Material Request" , mr , "custom_dc_refrance")
    print(mr)
...
def update_quotation_linked(doc , q ):
    ...
    for source_item in source_doc.items:
        new_item = frappe.new_doc("Quotation Item", parent=target_doc, parentfield="items", parenttype="Quotation")
